...
class StarTypePrediction(BaseModel):
    predicted_probabilities: dict = Field(
        description="Predicted probabilities for all classes.",
        example=pred_prob_example
    )
    predicted_class: str = Field(
        description="Predicted class based on the highest probability.",
        example="Brown Dwarf"
    )
    confidence_score: str = Field(
        description="Confidence score indicating the model's certainty in the predicted class.",
        example="65.9%"
    )
